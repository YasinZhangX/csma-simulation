...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] =Display(RecordBackoffTime,RecordSendTime, SendNodeIndex,RecordSendTimeFail, SendNodeIndexFail,TotalTime,NumberNodes,ConWindow);
figure(1);
...
for i=1:NumberNodes
    ...
    for k=1:SendNodeIndex(i)                                               %循环画出数据发送过程
     ...
     for k=1:SendNodeIndexFail(i)                                               %循环画出数据发送过程
        SendStart = RecordSendTimeFail(i,k,1);                             
        SendEnd = RecordSendTimeFail(i,k,2); 
        FrameLength = RecordSendTimeFail(i,k,3);
        Time = SendEnd - SendStart; 
        patch([SendStart SendEnd SendStart],[0 0 FrameLength],[.9 .0 .3]); %画出没有碰撞的数据发送过程
    end
