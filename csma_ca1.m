    ...
    if CollisionHandleFlag == TRUE&&t==ConWinEnd                           %有节点退避时间为0
        ...
        if n == 1                                                          %如果只有一个节点，可发送数据
            ...
            for  i = 1:n                                                   %多个点试图同时发送
                ...
                [RecordSendTimeFail SendNodeIndexFail]= RecordSend(RecordSendTimeFail,SendNodeIndexFail,j,SendStart(j),SendEndTimeTemp(j),FrameBuffer(j,2)); %争用期发生碰撞，发生碰撞的数据全部丢弃
