...
BackoffExponent = zeros(1, NumberNodes);                                   %退避指数
...
for i = 1:NumberNodes
    ...
    BackoffExponent(i) = MINBACKOFFEXPONENT;                               %初始化退避指数
end
RecordBackoffTime=zeros(NumberNodes,AllSlotTime);                          %记录站点的退避时间
RecordSendTime=zeros(NumberNodes,400,3);                                   %记录数据发送成功过程
            ...
            if CurBufferSize(i) < BufferSize - FrameLength(i)              %如果帧缓冲区还可以未满
                ...
                if HasFrameFlag(i) == FALSE                                %当缓冲区没有帧，此时有帧进入
                    ...
                    if ChannelBusyFlag== FALSE                             %当信道空闲
                        ...
                        BackoffTime(i)=SetBackoffTime(BackoffExponent(i)); %否则退避时间置为一随机时间
                    end
                end        
            end
            sign=sign+1;                                                   %不断生成新的帧
            if sign<NumberNodes*30
                ArrivalTime(i) = ceil(4*rand()) + 10 + t;
                FrameLength(i) = ceil(5+5*rand());
            end
        end
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if CollisionHandleFlag == TRUE&&t==ConWinEnd                           %有节点退避时间为0
        ...
        if n == 1                                                          %如果只有一个节点，可发送数据
            ...
            i = CollisionNodes(2);
            SendEndTimeTemp(i) = SendEndTime;                              %发送结束时间
            ...
            for  i = 1:n                                                   %多个点试图同时发送
                ...
                SendEndTimeTemp(j) = floor(SendStart(CollisionNodes(2))+ SIFS + DIFS + ACK + FrameBuffer(CollisionNodes(2),2));%计算发送完成时刻
                SendEndTime = max(SendEndTimeTemp);                        %求在碰撞过程最后结束的时间
            end
        end
        CollisionHandleFlag = FALSE;
    end
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      %步骤2-4：到达发送结束时刻就出栈
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      if ChannelBusyFlag == TRUE
          for i = 1:NumberNodes
              if t == SendEndTimeTemp(i)
                j = i;
                ...
                if k == 0                                                  %如果缓冲区无帧
                    ...
                    CountBackoff(j) = 0;                                   %退避次数置0
                else                                                       %还有数据分发送
                    CountBackoff(j) = CountBackoff(j) + 1;                 %退避次数加1
                    if CountBackoff(j) > MAXNUMBACKOFFS                    %退避次数超过最大值
                        CountBackoff(j) = 0;                               %退避次数置0
                        BackoffTime(j) = 0;                                %退避时间置0
                        [RecordSendTimeFail, SendNodeIndexFail] = ...
                        RecordSend(RecordSendTimeFail,SendNodeIndexFail,...
                        j,SendStart(j),SendEndTimeTemp(j),FrameBuffer(j,2)); % 争用期发生碰撞，且超过最大重试次数，发生碰撞的数据全部丢弃
                    else
